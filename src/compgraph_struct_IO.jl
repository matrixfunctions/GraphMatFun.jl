...
function export_compgraph(graph, fname; main_output=nothing,
    ...
    # USER extraction for windows and linux: https://stackoverflow.com/questions/27809812/get-current-username-in-julia-linux
    fname = abspath(fname)
    file = open(fname, "w+")
...
function import_compgraph(fname)
    ...
    while !eof(file)
        ...
        if startswith(line, "coeff1=") #Start of three lines defining :lincomb
            α = Vector{T}(undef,2)
            for i = 1:2
                num = replace(line,r".*=" => "") #remove everything upto and including "="
            ...
            for i = 1:2
                p[i] = Symbol(replace(subparts[i],r".*\*" => "")) #remove everything upto and including "*"
            end
            add_lincomb!(graph, target, α[1], p[1], α[2], p[2])


        elseif occursin(r"\*",line) #Assumes lincomb is handled above. Looks for "*". Found :mult
            parts = split(line,"=") # Split string on "="
            target = Symbol(parts[1])
            subparts = split(parts[2], "*")
            if endswith(subparts[2], ";")
