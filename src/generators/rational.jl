...
where p(A) and q(A) are polynomials defined by the coeficients `den_coeffs` and
...
also be passed to be modified accodringly, otherwise the return value `cref` is
empty for this call.
"""
